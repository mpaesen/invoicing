...
public class JDialogInvoiceDetail extends JDialog {
	...
	private InvoiceDetail createDetail() {
		...
		InvoiceDetail newDetail = new InvoiceDetail(invoice.getIdInvoice(),
				new Integer(jTextFieldInvDetLine.getText()),
				invProducts.get(jComboBoxProduct.getSelectedItem().toString()),
				new BigDecimal(jTextFieldInvQty.getText()),
				invUnits.get(getJComboBoxUnitOfMeasure().getSelectedItem()
						.toString()), getPrice(jComboBoxPrice.getSelectedItem()
						.toString()), vat, jTextAreaComments.getText(), true);
		return newDetail;
	}
	...
	private ComboBoxModel getJComboBoxPriceModel() {
		invPrices = PriceController.getProductPriceDetails(invProducts
				.get(getJComboBoxProduct().getSelectedItem().toString()),
				invoice.getInvDate());
	...
	private void initializeFields() {
		...
		if (detail != null) {
			...
			getJComboBoxUnitOfMeasure().setSelectedItem(
					ComboBoxHelper.getSelectedItem(invUnits,
			...
			jTextFieldLineTotalIncl.setText(calculateLineTotalIncl(
					calculateLineTotalExcl(detail), calculateVat(detail)));
	...
	private String calculateVat(InvoiceDetail detail) {
		String StrVatPerc = invVat.get(jComboBoxVatPercentage.getSelectedItem()
				.toString());
		double vatPerc = new BigDecimal(StrVatPerc).doubleValue()
				/ Figures.HUNDRED;
	...
	private String calculateLineTotalIncl(String excl, String vatAmt) {
		double total = new BigDecimal(excl).doubleValue()
				+ new BigDecimal(vatAmt).doubleValue();
		BigDecimal dec = new BigDecimal(total);
		dec = dec.setScale(2, BigDecimal.ROUND_HALF_UP);
		return dec.toString();
	}
